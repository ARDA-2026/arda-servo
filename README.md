...
평소엔 마지막으로 멈춘 각도에 가만히 있다가, 레이더(`arda-radar`)가 낙하를
확정한 좌표를 UDP로 받으면 그 방향으로 Jetson GPIO에 연결된 팬(pan,
수평 1축) 서보 모터를 움직여 확인한다. 사람을 계속 따라다니며 추적하는
용도가 아니라, 낙하 시점에만 그 지점을 보러 가는 용도다. **서보는 오직
레이더 낙하 좌표(트리거)를 받을 때만 움직인다** — dwell이 끝나도(시간
초과·열화상 포기·열화상 확정 어느 경우든) 홈 포지션으로 돌아가지 않고
그 자리에 그대로 멈춘 채 다음 트리거를 기다린다. 프로세스 시작 시
`center_deg`로 1회 이동하는 것만 예외다(초기 기준 위치 확보용).
...
1. 시작하면 즉시 홈 포지션(`center_deg`)으로 1회 이동해 정지한다 — 이후로는
   트리거 없이 스스로 움직이지 않는다.
2. `arda-radar`가 낙하를 확정하면 그 좌표를 `{"x", "y", "z", "fall": true,
   "confidence", "ts"}` JSON으로 UDP 전송한다 (기본 `127.0.0.1:9999`,
   `--no-servo-out`으로 비활성화 가능). `fall`이 `false`인 일반 추적 좌표는
   무시한다.
...
4. 그 각도에서 `dwell_seconds`(기본 10초 — `config/settings.yaml`에 값이
   ...
   지금 쫓는 후보보다 높은 새 낙하 좌표가 오면 예외적으로 그쪽을
   선점한다(아래 "확률 기반 선점" 참고).
5. dwell이 끝나면(시간 초과·열화상 포기·열화상 확정 무엇이든) 서보는
   홈으로 돌아가지 않고 그 각도에 그대로 멈춰 다음 낙하 트리거를 기다린다.

### 확률 기반 선점

arda-radar는 낙하마다 신뢰도(`confidence`, 0~1, 로지스틱 회귀 확률)를
함께 보낸다. dwell 중(4단계) 지금 쫓고 있는 후보보다 confidence가 더 높은
새 낙하 좌표가 오면, 진행 중이던 추적(열화상 보정 반영분 포함)을 그대로
버리고 그 즉시 새 좌표로 재조준해 dwell을 처음부터 다시 시작한다 — 더
유력한 낙하 후보가 나타났으면 기존에 보던 곳을 계속 붙잡고 있을 이유가
없기 때문이다. confidence가 같거나 낮은 새 좌표는 기존처럼 무시된다.
...
(열원을 놓쳤거나 관찰이 끝나면) dwell이 자연히 만료되어 그 자리에 멈춘다.
dwell 중이 아닐 때는 이 보정을 받지 않는다 — 레이더 트리거 없이 임의의
열원에 반응하지 않기 위해서다. `config/settings.yaml`의 `thermal_udp`
섹션을 통째로 지우면 이 기능은 비활성화되고 기존 동작만 남는다.
...
| `servo.center_deg` | 레이더 정면(azimuth 0°)에 대응하는 서보 각도이자 시작 시 1회 이동하는 초기 위치. dwell 종료 후에는 이 각도로 돌아가지 않는다 |
| `servo.invert` | 배선/장착 방향 때문에 좌우가 반대로 움직이면 `true` |
| `servo.dwell_seconds` | 낙하(`fall=true`) 좌표 수신 시 그 각도에서 정지할 시간(초). 끝나도 홈으로 돌아가지 않고 그 각도에 그대로 멈춘다. 0이면 정지 없이 즉시 종료(각도는 유지) |
